...
param(
  ...
  [string]$PasswordLogPath = 'passwords.log.enc',
...
if ($UseRandomPassword) {
  if ($PasswordLogKeyBase64) {
    try {
      ...
      "Username,Password_Protected" | Out-File -FilePath $PasswordLogPath -Encoding utf8
      Write-Log -Level 'Warning' -Message "Random passwords enabled; encrypted password log active" -Context @{ Path=$PasswordLogPath }
    } catch {
  ...
  if ($UseRandomPassword) {
    ...
    if ($PasswordKey) {
      try {
        ...
        "$username,$protected" | Out-File -FilePath $PasswordLogPath -Append -Encoding utf8
      } catch {
